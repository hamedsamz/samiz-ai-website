...
import { sendCodingCourseConfirmationEmail } from "../../../../../../lib/coding-course-confirmation-email";
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const sql = db();
  const rows = await sql`SELECT id, status, full_name AS "fullName", email FROM coding_course_registrations WHERE id = ${id}`;
  ...
  await sql`UPDATE coding_course_registrations SET status = ${status}, updated_at = ${Date.now()} WHERE id = ${id}`;

  if (status === "approved") {
    try {
      await sendCodingCourseConfirmationEmail(registration.email, registration.fullName);
      await sql`UPDATE coding_course_registrations SET confirmation_email_sent_at = ${Date.now()} WHERE id = ${id}`;
      return Response.json({ ok: true, message: "ثبت‌نام تأیید و ایمیل ورود به کانال ارسال شد." });
    } catch (error) {
      console.error("coding course approval email error", error);
