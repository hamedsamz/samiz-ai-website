...
type TipState = { y: number; time: number; armed: boolean; note: number };
      ...
      if (landmarker && video && video.readyState >= 2 && video.currentTime !== lastVideoTimeRef.current) {
        ...
        landmarks.forEach((hand, handIndex) => {
          ...
          FINGER_TIPS.forEach((tipIndex) => {
            ...
            const noteIndex = Math.max(0, Math.min(NOTES.length - 1, Math.floor(mapped.x / width * NOTES.length)));
            ...
            if (!previous || noteIndex !== previous.note || mapped.y < keyTop || mapped.y < previous.y - height * 0.014) armed = true;
            const downwardJump = previous ? mapped.y - previous.y : 0;
            const crossedLine = previous ? previous.y <= keyTop && mapped.y > keyTop : false;
            const deliberateTap = mapped.y > keyTop && downwardJump > height * 0.012;
            if (armed && now - lastStrike > 145 && (crossedLine || deliberateTap)) {
              playNoteRef.current(noteIndex);
              lastStrikeRef.current.set(id, now);
              armed = false;
            }
            tipStatesRef.current.set(id, { y: mapped.y, time: now, armed, note: noteIndex });
          });
    ...
    <main className={styles.page} dir="rtl">
      ...
      <section className={styles.shell}>
        <div className={styles.stage} ref={stageRef}>
          ...
          <div className={styles.noteGrid} aria-label="هفت نت اصلی موسیقی">
          ...
          {phase === "idle" && (
            <div className={styles.intro}>
              <span className={styles.eyebrow}>دو دست • ده انگشت • هفت نت</span>
              ...
              <small>همین حالا می‌توانی با لمس نت‌ها یا کلیدهای A تا L صدا را امتحان کنی.</small>
            </div>
          )}
          ...
          {phase === "ready" && (
            <div className={styles.hud}>
              ...
              <p>نوک انگشت را روی هر نت، کوتاه به سمت پایین حرکت بده</p>
            </div>
          )}
        </div>
        ...
        <div className={styles.instructions}>
          ...
          <div><b>۲</b><span><strong>انگشت را روی نت ببر</strong><small>هر ستون رنگی یکی از نت‌های اصلی موسیقی است.</small></span></div>
