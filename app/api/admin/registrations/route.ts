...
export async function GET() {
  ...
  const rows = await db()`SELECT id, slot_id AS "slotId", full_name AS "fullName", phone, receipt_name AS "receiptName", receipt_type AS "receiptType", status, created_at AS "createdAt" FROM registrations ORDER BY created_at DESC`;
