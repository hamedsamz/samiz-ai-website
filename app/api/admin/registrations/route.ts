...
export async function GET() {
  ...
  const rows = await db()`SELECT id, slot_id AS "slotId", full_name AS "fullName", phone, email, receipt_name AS "receiptName", receipt_type AS "receiptType", status, created_at AS "createdAt", confirmation_email_sent_at AS "emailSentAt" FROM registrations ORDER BY created_at DESC`;
