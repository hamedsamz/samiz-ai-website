...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const rows = await challengeDb()`SELECT receipt_name AS "receiptName", receipt_type AS "receiptType", receipt_data AS "receiptData" FROM calorie_challenge_registrations WHERE id = ${id}`;
