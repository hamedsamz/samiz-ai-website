...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.action === "single") {
      ...
      await sendAiVideoCourseConfirmationEmail(row.email, row.fullName);
      await db()`UPDATE ai_video_course_registrations SET confirmation_email_sent_at = ${Date.now()} WHERE id = ${row.id}`;
      return Response.json({ message: "ایمیل تأیید ارسال شد." });
    }
    if (body.action === "bulk") {
      const rows = await db()`SELECT id, full_name AS "fullName", email FROM ai_video_course_registrations WHERE status = 'approved' AND confirmation_email_sent_at IS NULL ORDER BY created_at LIMIT 100` as Array<{ id: string; fullName: string; email: string }>;
      if (!rows.length) return Response.json({ error: "ایمیل ارسال‌نشده‌ای وجود ندارد." }, { status: 409 });
      await sendAiVideoCourseConfirmationBatch(rows.map(row => ({ email: row.email, fullName: row.fullName })));
      const now = Date.now();
      const sql = db();
    ...
    console.error("ai video course confirmation email error", error);
    const missingChannel = error instanceof Error && error.message.includes("AI_VIDEO_COURSE_CHANNEL_URL");
