...
export async function POST(request: Request) {
  ...
  try {
    ...
    const sql = db();
    const duplicate = await sql`SELECT id FROM registrations WHERE phone = ${phone} AND status IN ('pending', 'approved') LIMIT 1`;
    if (duplicate.length) return Response.json({ error: "با این شماره تماس قبلاً ثبت‌نام شده است." }, { status: 409 });
    ...
    const inserted = await sql`WITH claimed AS (
      ...
      RETURNING id
    ) INSERT INTO registrations (id, slot_id, full_name, phone, receipt_name, receipt_type, receipt_data, status, created_at, updated_at)
      SELECT ${registrationId}, id, ${fullName}, ${phone}, ${receipt.name.slice(0, 160)}, ${receipt.type}, ${receiptData}, 'pending', ${now}, ${now} FROM claimed RETURNING id`;
